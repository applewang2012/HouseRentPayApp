...
public class DownloadManager {
	...
	public static class Request {
		...
		public Request setPackageName(String packageName) {
			mRequestPackageName = packageName;
			return this;
		}
		 ...
		 * {@link com.market.android.download.providers.NotificationManager}. By
		...
		ContentValues toContentValues(String packageName) {
			...
			//add for appstore
			values.put(Downloads.COLUMN_NOTIFICATION_EXTRAS, mRequestIconUrl);
			//end
			...
			values.put(Downloads.COLUMN_NO_INTEGRITY, false);
	...
	public long enqueue(Request request) {
		...
		values.put(Downloads.COLUMN_MD5, request.mRequestMD5);
//		if (request.mSourceType == com.market.android.util.Constants.DOWNLOAD_FROM_OTA) {
//			values.put(Downloads.COLUMN_DESTINATION,
...
//		}
	...
	public int cancelDownload(long... ids) {
        ...
        ContentValues values = new ContentValues();
        values.put(Downloads.COLUMN_STATUS, Downloads.STATUS_CANCELED);
        return mResolver.update(mBaseUri, values, getWhereClauseForIds(ids),
                getWhereArgsForIds(ids));
    }
	...
	public static boolean isStatusRunning(int status) {
		...
		return status == Downloads.STATUS_RUNNING;
	}
    ...
    public static boolean isStatusPending(int status) {
        return status == Downloads.STATUS_PENDING 
