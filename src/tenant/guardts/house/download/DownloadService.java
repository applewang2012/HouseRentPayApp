...
public class DownloadService extends Service {
	...
	for (int i = 0; i < files.length; i++) {
	    if (files[i].getName().equals(DownloadConstants.KNOWN_SPURIOUS_FILENAME)) {
		continue;
	    }
	    if (files[i].getName().equalsIgnoreCase(
		    DownloadConstants.RECOVERY_DIRECTORY)) {
		continue;
	    }
	...
	if (cursor.moveToFirst()) {
	    int numDelete = cursor.getCount() - DownloadConstants.MAX_DOWNLOADS;
