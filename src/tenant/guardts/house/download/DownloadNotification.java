...
class DownloadNotification {
    ...
    private void updateActiveNotification(Collection<DownloadInfo> downloads) {
        ...
        for (NotificationItem item : mNotifications.values()) {
            ...
            LogUtil.w("mingguo", "DownloadNotification   intent download receiver  progresss  "+getDownloadingText(item.mTotalTotal, item.mTotalCurrent));
        ...
        for (DownloadInfo download : downloads) {
            ...
            if (Downloads.isStatusError(download.mStatus)) {
                ...
                if (download.mDestination == Downloads.DESTINATION_EXTERNAL) {
                    intent = new Intent(DownloadConstants.ACTION_OPEN);
                } else {
                    intent = new Intent(DownloadConstants.ACTION_LIST);
                }
            }
            ...
            intent = new Intent(DownloadConstants.ACTION_HIDE);
            intent.setClassName(mContext.getPackageName(),
                    DownloadReceiver.class.getName());
            intent.setData(contentUri);
