...
public class DownloadAdapter extends CursorAdapter {
    ...
    public View newView() {
	DownloadStartItem view = (DownloadStartItem) LayoutInflater.from(mContext)
	...
	if (!(convertView instanceof DownloadStartItem)) {
	    return;
	}

	long downloadId = mCursor.getLong(mIdColumnId);
	((DownloadStartItem) convertView).setDownloadId(downloadId);
	...
	mDownloadSelectionListener.downloadStatus(status);
